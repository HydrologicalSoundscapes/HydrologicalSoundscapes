...
export function meanMonthlyPS(station, old_PS=null, arrangement_id) {
    ...
    if (!old_PS) {
        const chart = BarChart(data_to_plot, {
            x: (d, i) => d[0],
            y: (d, i) => d[1],
            color: "lightblue", height: 200, yDomain: [0, 0.5], yFormat: ".0%",  marginLeft: 30, 
        })
...
export function maxMonthlyPS(station, old_PS, arrangement_id) {
    ...
    if (!old_PS) {
        const chart = BarChart(data_to_plot, {
            x: (d, i) => d[0],
            y: (d, i) => d[1],
            color: "lightblue", height: 200,yDomain: [0, 1],yFormat: ".0%",  marginLeft: 30, 
        })
