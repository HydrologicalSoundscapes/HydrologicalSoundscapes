...
import {
  computeMeanMonthlyPart,
  computeMaxMonthlyPart,
  computeMinMonthlyPart,
  computeDrumMonthlyPart,
} from "./sounds";
...
export function meanMonthlyPS(station, old_PS = null, configuration) {
  ...
  const sound_scale_id = ARRANGEMENTS.find(
  ...
  if (!old_PS) {
    ...
    const part = computeMeanMonthlyPart(
      data_medium,
      data_volume,
      chart.highlight,
      sound_scale,
      configuration.inverted_pitch
    );
